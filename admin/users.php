					...
					<?php
				        ...
				        if ($result = $conn->query("SELECT * FROM users")){
				        	if ($result->num_rows != 0){
							   ...
							   echo'<tr>
									<td><a href="http://'. $parse['host'] .'/users/user.php?='. $row['username'] . '">'.  $row['username'] .'</a></td>
									<td>' . $row['rank'] . '</td>
									<td>' . $row['unit'] . '</td>
									<td>' . $row['position'] . '</td>
									<td>' . $row['status'] . '</td>
									<td><a href="edit/users.php?id=' . $row['id'] . '">Edit</a></td>
									<td><a href="delete/users.php?id=' . $row['id'] . '">Delete</a></td>
									</tr>';
						        }
								echo '
								</tbody>
