	...
	<?php
		...
		if (isset($_POST['submit'])){
			if (is_numeric($_POST['id'])){
				...
				$link = htmlentities($_POST['link'], ENT_QUOTES);
					if ($stmt = $conn->prepare("UPDATE navbar SET title = ?, link = ? WHERE id=?")){
			...
			if (is_numeric($_GET['id']) && $_GET['id'] > 0){
				$id = $_GET['id'];
				if($stmt = $conn->prepare("SELECT * FROM navbar WHERE id = ?")){
		...
		if (isset($_POST['submit'])){
			...
			if ($title == '' || $link == ''){
				...
				if ($stmt = $conn->prepare("INSERT navbar (title, link) VALUES (?, ?)")){
